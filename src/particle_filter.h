...
public:
    ...
    /**
     * Return double vector of CRTV model for given params
     */
    vector<double> CRTVModel(double delta_t, const vector<double>& x, const vector<double>& u, const double std_pos[]);

    /**
     * Convert local coordinate(VEHICLE'S coordinate) to global coordinate(MAP'S coordinate)
     */
    const LandmarkObs localToGlobal(const LandmarkObs& obs, const Particle& p);
