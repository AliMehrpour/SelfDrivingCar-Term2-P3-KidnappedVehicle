 ...
 *
 *  Updated on: Jun 10, 2017 By Ali Mehrpour
 */
...
void ParticleFilter::prediction(double delta_t, double std_pos[], double velocity, double yaw_rate)
{
  ...
  {
    auto p = particles[i];
    vector_t x_pred = CRTVModel(delta_t, {p.x, p.y, p.theta}, {velocity, yaw_rate}, std_pos);
		...
		std::vector<LandmarkObs> observations, Map map_landmarks)
{
  ...
  for(unsigned p_ctr=0; p_ctr < particles.size(); p_ctr++)
  {
    ...
    {
      auto obs_global = localToGlobal(obs_lm, p);
...
vector<double> ParticleFilter::CRTVModel(double delta_t, const vector<double>& x, const vector<double>& u, const double std_pos[])
{
...
const LandmarkObs ParticleFilter::localToGlobal(const LandmarkObs& obs, const Particle& p)
{
  LandmarkObs out;
...
inline const double ParticleFilter::gaussian2d(const LandmarkObs& obs, const LandmarkObs &lm, const double sigma[])
{
